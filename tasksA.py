...
# Use a fixed data directory (all files must be under /data)
DATA_DIR = "/data"
...
def task_A1_install_and_run_datagen():
    email = "22f1001780@ds.study.iitm.ac.in"  # Use your email
...
# A2: Format /data/format.md using prettier@3.4.2 (via npx)
...
# A3: Count Wednesdays in /data/dates.txt and write number to /data/dates-wednesdays.txt
...
# A5: Write first line of 10 most recent .log files in /data/logs/ to /data/logs-recent.txt
...
# A6: Extract first H1 from each Markdown file in /data/docs/ and create index file /data/docs/index.json.
...
# A9: Find most similar pair of comments from /data/comments.txt using embeddings.
def task_A9_find_similar_comments():
    ...
    # Get embeddings using OpenAI Embeddings API (using a supported model)
...
# A10: Compute total sales for Gold tickets from /data/ticket-sales.db.
